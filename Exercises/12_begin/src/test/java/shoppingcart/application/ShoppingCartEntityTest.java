...
import static shoppingcart.domain.ShoppingCartEvent.CheckedOut;
...
public class ShoppingCartEntityTest {
    ...
    @Test
    void testCheckoutPersistsEventAndPreventsFurtherChanges() {
        var testKit = EventSourcedTestKit.of(ShoppingCartEntity::new);
        testKit.method(ShoppingCartEntity::addItem).invoke(akkaTshirt);

        var result = testKit.method(ShoppingCartEntity::checkout).invoke();
        assertEquals(Done.getInstance(), result.getReply());

        var event = result.getNextEventOfType(CheckedOut.class);
        assertNotNull(event);

        var state = testKit.getState();
        assertTrue(state.checkedOut());

        var addResult = testKit.method(ShoppingCartEntity::addItem).invoke(blueJeans);
        assertTrue(addResult.isError());
        assertEquals("Cart is already checked out.", addResult.getError());

        var removeResult = testKit.method(ShoppingCartEntity::removeItem).invoke("akka-tshirt");
        assertTrue(removeResult.isError());
        assertEquals("Cart is already checked out.", removeResult.getError());

        var checkoutAgain = testKit.method(ShoppingCartEntity::checkout).invoke();
        assertTrue(checkoutAgain.isError());
        assertEquals("Cart is already checked out.", checkoutAgain.getError());
    }
}