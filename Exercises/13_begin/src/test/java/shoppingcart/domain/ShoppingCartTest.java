...
class ShoppingCartTest {
  ...
  void cartImmutabilityIsPreserved() {
    ...
    var event = new ShoppingCartEvent.ItemAdded(new ShoppingCart.LineItem("p1", "Akka Hoodie", 1));
  ...
  void itemsAreSortedByProductId() {
    ...
    var event =
        new ShoppingCartEvent.ItemAdded(new ShoppingCart.LineItem("a-product", "Akka Hoodie", 2));
  ...
  // --- Tests for onItemRemoved ---

  @Test
  void itemIsRemovedIfItExists() {
    var hoodie = new ShoppingCart.LineItem("p1", "Akka Hoodie", 1);
    var mug = new ShoppingCart.LineItem("p2", "Akka Mug", 1);
    var cart = new ShoppingCart("cart-1", List.of(hoodie, mug), false);

    var event = new ShoppingCartEvent.ItemRemoved("p1");
    var updatedCart = cart.onItemRemoved(event);

    assertEquals(1, updatedCart.items().size());
    assertEquals("p2", updatedCart.items().get(0).productId());
  }

  @Test
  void cartRemainsUnchangedWhenItemNotFound() {
    var item = new ShoppingCart.LineItem("p1", "Akka Hoodie", 1);
    var cart = new ShoppingCart("cart-1", List.of(item), false);

    var event = new ShoppingCartEvent.ItemRemoved("nonexistent");
    var updatedCart = cart.onItemRemoved(event);
  ...
  @Test
  void removingLastItemResultsInEmptyCart() {
    var item = new ShoppingCart.LineItem("p1", "Akka Hoodie", 1);
    var cart = new ShoppingCart("cart-1", List.of(item), false);
  ...
  @Test
  void checkedOutReturnsUpdatedState() {
    var originalCart = new ShoppingCart("cart-1", List.of(), false);

    var checkedOutCart = originalCart.onCheckedOut();

    assertFalse(originalCart.checkedOut());
    assertTrue(checkedOutCart.checkedOut());
    assertEquals("cart-1", checkedOutCart.cartId());
    assertEquals(originalCart.items(), checkedOutCart.items());
  }
}
