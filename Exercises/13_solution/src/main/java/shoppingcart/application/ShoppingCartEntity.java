...
public class ShoppingCartEntity extends EventSourcedEntity<ShoppingCart, ShoppingCartEvent> {
    ...
    public ShoppingCart applyEvent(ShoppingCartEvent event) {
        return switch (event) {
            ...
            case ShoppingCartEvent.ItemRemoved itemRemoved -> currentState().onItemRemoved(itemRemoved);
            case ShoppingCartEvent.CheckedOut ignored -> currentState().onCheckedOut();
        };
    }
    ...
    public Effect<Done> checkout() {
        ...
        var event = new ShoppingCartEvent.CheckedOut();
        return effects()
            .persist(event)
            .thenReply(newState -> Done.getInstance());
    }
